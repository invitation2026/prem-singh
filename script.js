document.addEventListener('DOMContentLoaded', () => {
    ...
    // ========== 1. INSTANT LOADER & SMOOTH HIDE ==========
  const loader = document.getElementById('loader');
  const minDisplayTime = 1000; // Minimum 1 sec dikhana hai taaki animation smooth lage
  const domReadyTime = Date.now();

  function hideLoaderAndStart() {
    const elapsed = Date.now() - domReadyTime;
    const remaining = Math.max(0, minDisplayTime - elapsed);

    setTimeout(() => {
      loader.classList.add('hidden');
      document.body.classList.add('loaded'); // Enable scroll
      // Confetti loader ke baad start hoga
      setTimeout(() => createConfetti(), 200);
    }, remaining);
  }

  // DOM ready hote hi trigger karo (fonts/images ka wait nahi karega)
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', hideLoaderAndStart);
  } else {
    hideLoaderAndStart();
  }

  // Fallback: Agar koi heavy image late load ho, phir bhi loader hide ho jaye
  ...
  // ========== 3. MOBILE DETECTION & PERFORMANCE TUNING ==========
  ...
  class Particle {
    ...
    draw() {
      ctx.beginPath();
      ctx.arc(this.x / (isMobile ? 1 : (window.devicePixelRatio||1)), this.y / (isMobile ? 1 : (window.devicePixelRatio||1)), this.size, 0, Math.PI * 2);
  ...
  // ========== 6. 3D TILT (DESKTOP) / SCALE (MOBILE) ==========
  ...
  // ========== 7. SCROLL HINT AUTO-HIDE ==========
  const scrollHint = document.getElementById('scroll-hint');
  let userScrolled = false;
  function hideScrollHint() {
    if (!userScrolled && scrollHint) { scrollHint.classList.add('hidden'); userScrolled = true; }
  }
  if (scrollHint) {
